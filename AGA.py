...
def initPopulation(popsize,numOfWorker,numOfProduct,maxSeru):#初始化种群，要求生成的个体没有重复个体(目标值相同个体)
    ...
    while(len(solution)<popsize):
        ...
        pop_cost=computeTotalcost(pop,demand,TC,FC,A,sc,ec)
        if pop_cost not in cost_list:
            cost_list.append(pop_cost)
            solution.append([pop,pop_cost])
    #print('solution',solution)
    solution.sort(key= lambda x: x[1])#按成本从小到大排序,带成本的
    #print('排序后solution',solution)
    population=[]
    for j in solution:
...
def tournament_selection(solution):#二元锦标赛选择算子，每次选择2个个体，将两者中较优的个体留下
    #个体索引列表    
...
def next_Population(solution):#输入父代及其成本，产生子代，父代与子代合并产生下一代种群及其成本
    newsolution=[]
    while len(newsolution)<len(solution):#产生子代
        parents=[]
        for i in range(2):#选择两个个体
           parents.append(tournament_selection(solution))
...
def main():
    ...
    for i in range(2,maxSeru+1):
        ...
        while iterator<maxIterator:#解保持不变的代数
            print('##########进入循环',iterator)
            newsolution=next_Population(solution)#按成本从小到大排好序的种群
            print('………………………………………………下一代………………………………………………',newsolution)
            if newsolution[0][1]==bestcost_list[-1]:
                iterator+=1
            else:
                iterator=0
            bestcost_list.append(newsolution[0][1])
            print('bestcost_list',bestcost_list)
            solution=newsolution
        result.append([newsolution[0][0],newsolution[0][1]])
